...
public class WalletController {
	...
	public String saveTopUpAmt(@PathVariable("id") Integer id, @RequestParam("topUpAmt") double amount, Model model) {
...
//			model.addAttribute("TotransactionId", TotransactionId);
	...
	public String deleteWallet(@PathVariable("id") Integer id, RedirectAttributes RedirectAttributes, Model model) {

		Wallet wallet = walletRepository.getById(id);
		if (wallet.getTotalAmount() != 0) {
			RedirectAttributes.addFlashAttribute("warning", "Wallet ID: " + id + "is not successfully deleted.");
...
//		}
	...
	public String saveWalletLimit(@PathVariable("id") Integer id,
		...
		if (updatedWalletLimit == oldWalletLimit) {
			notifications.setMessage("No changes was made for your wallet ID: " + id + ".");
	...
	public String withdrawWallet(Model model) {
